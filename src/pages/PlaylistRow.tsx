...
import { loadVoters, useVoters } from '../lib/voters'
...
export default function PlaylistRow({
  ...
  const voters = useVoters(playlist.id)

  const saved = favoriteIds.has(playlist.id)
  const voted = upvotedIds.has(playlist.id)
  const votes = counts.get(playlist.id) ?? 0

  // Asked for only when someone opens the list. Fetching them for every row on
  // screen would be hundreds of profiles nobody wanted to read.
  function toggleVoters(): void {
    if (!showVoters) void loadVoters(playlist.id)
    setShowVoters((open) => !open)
  }
  ...
  return (
    <>
        ...
        <span className="row-meta">
          ...
          <span className="row-actions">
            ...
            {/* Hidden at zero rather than showing a 0 on every row: an empty
                count is noise, and the arrow alone already invites the first
                vote. Its own button, separate from the arrow, so reading who
                voted never casts one by accident. */}
            {votes > 0 && (
              <button
                type="button"
                className={showVoters ? 'row-votes open' : 'row-votes'}
                aria-expanded={showVoters}
                aria-label={`${votes} vote${votes > 1 ? 's' : ''}, voir qui a voté`}
      ...
      {showVoters && (
        <div className="row-voters">
          {(!voters || voters.status === 'loading') && <span className="row-voters-note">Chargement…</span>}
          {voters?.status === 'failed' && <span className="row-voters-note">Liste indisponible.</span>}
          {voters?.status === 'ready' &&
            (voters.voters.length === 0 ? (
              // Not the same sentence an empty list would suggest: the votes are
              // there, it is the names that nobody chose to show.
              <span className="row-voters-note">Personne n'a rendu son vote public sur cette playlist.</span>
            ) : (
              voters.voters.map((voter) => (
                <span key={voter.userId} className="row-voter">
                  {voter.displayName}
                </span>
              ))
            ))}
